...
class HTTPAuthTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        @basic_verify_auth.check_honeytoken
        def verify_honeytoken(auth):
            if not auth or not auth.username:
                return False
            elif auth.username == "h" and auth.password == "bee":
                return True
            return False
        ...
        def basic_honey_route():
            return 'basic_verify_auth: honey user: ' + str(basic_verify_auth.username())

        @app.route('/basic-verify')
        @basic_verify_auth.login_required(honey=basic_honey_route)
    ...
    def test_honey_auth_honey_login(self):
    ...
    def test_honey_auth_honey_login_invalid(self):
        creds = base64.b64encode(b'h:baa').decode('utf-8')
        response = self.client.get(
                '/basic-verify', headers={'Authorization': 'Basic ' + creds})
        self.assertEqual(response.status_code, 403)
        self.assertTrue('WWW-Authenticate' in response.headers)
